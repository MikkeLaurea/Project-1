...
function addTask() {
    ...
    if (taskText === "") {
        displayError("Task can't be empty."); // Näytä virheviesti, jos tehtävä on tyhjä
        return;
    } else if (taskText.length < 3) {
        displayError("Task is too short, it needs to be atleast 3 characters long."); // Näytä virheviesti, jos tehtävä on liian lyhyt
        return;
    }
